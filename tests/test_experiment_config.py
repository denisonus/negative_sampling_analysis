...
class ExperimentConfigTests(unittest.TestCase):
    ...
    def test_ml1m_compact_config_resolves_dataset_preset(self):
        config = resolve_config({"dataset": "ml-1m"})

        self.assertEqual(config["dataset"], "ml-1m")
        self.assertFalse(config["implicit_feedback"])
        self.assertEqual(config["min_rating"], 4)
        self.assertEqual(config["topk"], [10, 20])
        self.assertEqual(config["valid_metric"], "NDCG@20")

    def test_unknown_dataset_fails_clearly(self):
        with self.assertRaisesRegex(ValueError, "Unsupported dataset 'unknown'"):
            resolve_config({"dataset": "unknown"})
    ...
    def test_all_experiment_configs_resolve(self):
        for path in sorted(Path("config/exp").rglob("*.yaml")):
            with self.subTest(config=str(path)):
                config = load_config(path)
                self.assertIn(config["dataset"], {"ml-100k", "ml-1m", "gowalla-1m"})
