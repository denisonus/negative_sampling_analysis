...
def plot_metric_comparison(results, metric='ndcg@10', output_path=None):
    ...
    if 'statistics' in results:
        ...
        values = [stats_data[s]['metrics'].get(metric, {}).get('mean', 0) for s in strategies]
        errors = [stats_data[s]['metrics'].get(metric, {}).get('std', 0) for s in strategies]
    else:
    ...
    bars = plt.bar(strategies, values, color='steelblue', edgecolor='black', 
...
def plot_all_metrics(results, metrics=['ndcg@10', 'recall@10', 'hit@10', 'mrr@10'], output_path=None):
    ...
    for i, metric in enumerate(metrics):
        ...
        ax.bar(x + offset, values, width, label=metric, color=colors[i],
               yerr=errors if has_errors else None, capsize=3 if has_errors else 0)
...
def plot_training_curves(results, output_path=None):
    ...
    for idx, (strategy, runs) in enumerate(raw_results.items()):
        ...
        if all_valid and all_valid[0]:
            ...
            axes[1].fill_between(epochs, mean_valid - std_valid, mean_valid + std_valid,
                                 alpha=0.2, color=colors[idx])
...
def plot_timing_comparison(results, output_path=None):
    ...
    sampling_times = [stats_data[s]['timing']['sampling_time']['mean'] for s in strategies]
    training_times = [stats_data[s]['timing']['training_time']['mean'] for s in strategies]
    
    ax.bar(x - width/2, sampling_times, width, label='Sampling Time', color='coral')
    ax.bar(x + width/2, training_times, width, label='Training Time', color='steelblue')
...
def plot_convergence_speed(results, target_metric='ndcg@10', threshold_percentile=0.9, output_path=None):
    ...
    bars = ax.bar(strategies, means, yerr=stds, capsize=5, color='teal', edgecolor='black')
...
def statistical_significance_test(results, metric='ndcg@10', baseline='uniform'):
    ...
    for strategy in stats_data:
        ...
        strategy_values = stats_data[strategy]['metrics'].get(metric, {}).get('values', [])
        ...
        print(f"{strategy:<15} diff={mean_diff:+.4f}  t={t_stat:.3f}  p={p_value:.4f} {significance}")
...
def create_latex_table(results, metrics=['ndcg@10', 'recall@10', 'hit@10', 'mrr@10']):
    ...
    lines = [
        ...
        "\\caption{Comparison of Negative Sampling Strategies}", "\\label{tab:comparison}",
        "\\begin{tabular}{l" + "c" * len(metrics) + "}", "\\toprule",
        " & ".join(["Strategy"] + [m.upper() for m in metrics]) + " \\\\", "\\midrule"
    ]
...
def generate_full_report(results_file, output_dir='results'):
    ...
    for metric in ['ndcg@10', 'recall@10', 'hit@10', 'mrr@10']:
        plot_metric_comparison(results, metric=metric, 
                              output_path=os.path.join(output_dir, f'{metric.replace("@", "_")}.png'))
    
    # Training curves
    if 'raw_results' in results:
        plot_training_curves(results, output_path=os.path.join(output_dir, 'training_curves.png'))
        plot_convergence_speed(results, output_path=os.path.join(output_dir, 'convergence.png'))
    
    # Timing
    if 'statistics' in results:
        plot_timing_comparison(results, output_path=os.path.join(output_dir, 'timing.png'))
